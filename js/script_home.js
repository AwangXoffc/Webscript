    ...
    window.requestAnimationFrame(() => {
        ...
        if (recentScripts.length === 0) {
            ...
            <div class="snap-center w-full flex flex-col items-center justify-center py-10 px-4 text-center border border-dashed border-[#334155] rounded-3xl bg-[#0f172a] mx-auto reveal-on-scroll is-visible" style="min-width: 100%;">
                ...
                <button onclick="window.location.href='/view/sc.html'; playSfx('pop')" class="px-6 py-3 bg-[#1e3a8a] rounded-xl text-white text-[10px] font-black uppercase shadow-md active:scale-95 transition-all hover:bg-[#1e40af]">
...
function transformHomeSearch() {
    ...
    const newButtonHtml = `
    <button id="btn-home-action" onclick="window.location.href='/view/sc.html'; playSfx('pop')" class="w-full relative overflow-hidden bg-[#0f172a] border border-[#334155] rounded-2xl py-5 px-6 flex items-center justify-between group hover:border-[#60a5fa] transition-all shadow-lg active:scale-95">
...
if (typeof CONFIG !== 'undefined' && CONFIG.items) {
    if (!window.isScriptRendered) {
        window.renderHomeScripts(CONFIG.items);
    }
}
